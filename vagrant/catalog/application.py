...
def videogameLibraryJSON(game_machine_id):
    game_machine = session.query(GameMachine).filter_by(id = game_machine_id).one()
    items = session.query(VideoGame).filter_by(game_machine_id = game_machine_id).order_by("name").all()
    return jsonify(ConsoleGames = [i.serialize for i in items])


@app.route('/gamemachine/<int:game_machine_id>/videogame/<int:video_game_id>/JSON')
...
def showGameMachines():
  ...
  if 'username' not in login_session:
    ...
    return render_template('gamemachines.html', game_machines = game_machines,
                          users = users)
...
def newGameMachine():
  ...
  if request.method == 'POST':
      newGameMachine = GameMachine(manufacturer = request.form['manufacturer'],
...
#Edit a game machine
@app.route('/gamemachine/<int:game_machine_id>/edit/', methods = ['GET', 'POST'])
...
def deleteGameMachine(game_machine_id):
  if 'username' not in login_session:
    return redirect('/login')
  gamemachineToDelete = session.query(
                              GameMachine).filter_by(id = game_machine_id).one()
  if gamemachineToDelete.user_id != login_session['user_id']:
    return "<script>function myFunction() {alert('You are not authorized' \
    ' to delete this console.');}</script><body onload='myFunction()''>"
  if request.method == 'POST':
    session.delete(gamemachineToDelete)
    flash('%s Successfully Deleted' % gamemachineToDelete.name)
    session.commit()
    return redirect(url_for('showGameMachines', game_machine_id = game_machine_id))
  else:
...
@app.route('/gamemachine/<int:game_machine_id>/videogame/new/',methods=['GET','POST'])
...
@app.route('/gamemachine/<int:game_machine_id>/videogame/<int:video_game_id>/edit',
           methods=['GET','POST'])
def editVideoGame(game_machine_id, video_game_id):
  ...
  if request.method == 'POST':
      ...
      return redirect(url_for('showVGLib', game_machine_id = game_machine_id))
  else:
      return render_template('editvglibitem.html',
                              game_machine_id = game_machine_id,
                              video_game_id = video_game_id, item = editedItem)


#Delete a video game item
@app.route('/gamemachine/<int:game_machine_id>/videogame/<int:video_game_id>/delete',
            methods = ['GET','POST'])
